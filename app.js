...
let win = false;
let rules =
    "Welcome to Brainvita!<p>The objective of this one-person game is to remove all but one marble from the board and the last marble must end up in the centre<p>To remove a marble, you must move another marble over it and into an empty hollow. The marbles can not move diagonally or skip over multiple marbles<p>Use the buttons on top left to rewind a move/change the game view (not available on smaller screens)";
let continueOption =
    "You have an unfinished game. Would like to continue or start a new one?";
...
function changeView() {
    ...
    if (nudeSkin) {
        ...
        $(".brainvita").addClass("nude-brainvita");
        $(".square").addClass("nude-square");
        myStorage.setItem("32", "nude");
    } else {
        changeFavicon("2");
        $(".brainvita").removeClass("nude-brainvita");
        $(".square").removeClass("nude-square");
        myStorage.setItem("32", "red");
    }
}
...
function createGrid() {
    ...
    if (myStorage.getItem(32) == "nude" && !backingUp) {
...
function createModal(t, buttons = "single") {
    ...
    if (buttons == "double") {
        ...
        const $continue = $("<button>")
            ...
            .appendTo($buttonsDiv)
            .on("click", loadGame.bind(event, $modal));
        const $start = $("<button>")
            .text("New Game")
            .addClass("modal-button")
            .appendTo($buttonsDiv)
            .on("click", setUpNewGame.bind(event, $modal));
    } else {
        const $close = $("<button>")
            .text("Close")
            .addClass("modal-button")
...
function loadGame($modal) {
    ...
    let positions = myStorage.getItem(33 - myStorage.length).split(","); //34 bc 32 is game view info
    // console.log(positions);
    for (let position of positions) {
        const $square = $(`#${position}.square`);
        const $marble = $("<img>")
...
function createDragDrop() {
    $(".marble").draggable({
        ...
        drag: function(event, marble) {
            if (isMobileTablet()) {
                //if touch device, increase size more
    ...
    $(".square").droppable({
        ...
        accept: function(marble) {
            ...
            if (
                ((Math.abs(midi - sidi) == 2 && Math.abs(midj - sidj) == 0) ||
                    (Math.abs(midi - sidi) == 0 && Math.abs(midj - sidj) == 2)) &&
                $(`#${jumpi}${jumpj}.square`)
                .attr("class")
                .includes("ui-droppable-disabled")
            ) {
                return true;
            } else {
                return false;
...
function checkWin() {
    ...
    if (!legalMovesLeft()) {
        ...
        if (
            marblesOnBoard == 1 &&
            $("#33.square").attr("class").includes("ui-droppable-disabled")
        ) {
            winSound.play();
            myStorage.clear();
            // if (trainingMode) {
